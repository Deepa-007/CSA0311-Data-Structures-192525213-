...
class TrafficGraph:
    ...
    def generate_random_city(self, num_nodes=20, num_edges=40):
        """
        Generates a connected random city road graph with up to 50 nodes and 100 roads.
        """
        self.clear()
        num_nodes = max(10, min(50, num_nodes))
        ...
        for i in range(len(shuffled) - 1):
            G_temp.add_edge(shuffled[i], shuffled[i+1])

        # Add extra random edges up to num_edges
        non_edges = list(nx.non_edges(G_temp))
        random.shuffle(non_edges)
        needed = num_edges - len(G_temp.edges())
        for i in range(min(needed, len(non_edges))):
            u, v = non_edges[i]
            G_temp.add_edge(u, v)
        ...
        for n in G_temp.nodes():
            x, y = pos_dict[n]
            self.add_intersection(n, n, (round(x * 12, 2), round(y * 12, 2)))
...
class EmergencyRouteCommanderApp:
    ...
    def on_generate_random_city_dialog(self):
        ...
        tk.Label(dialog, text="🎲 Random City Configuration", font=('Segoe UI', 11, 'bold'), fg=THEME['accent_cyan'], bg=THEME['bg_panel']).pack(pady=(10, 5))

        f_nodes = tk.Frame(dialog, bg=THEME['bg_panel'])
        f_nodes.pack(fill=tk.X, padx=20, pady=4)
        tk.Label(f_nodes, text="Number of Intersections (10 - 50):", font=('Segoe UI', 9)).pack(anchor='w')
        slider_nodes = tk.Scale(f_nodes, from_=10, to=50, orient=tk.HORIZONTAL, bg=THEME['bg_panel'], fg=THEME['text_bright'], highlightthickness=0)
        slider_nodes.set(20)
        slider_nodes.pack(fill=tk.X)

        f_edges = tk.Frame(dialog, bg=THEME['bg_panel'])
        f_edges.pack(fill=tk.X, padx=20, pady=4)
        tk.Label(f_edges, text="Number of Roads (15 - 100):", font=('Segoe UI', 9)).pack(anchor='w')
        slider_edges = tk.Scale(f_edges, from_=15, to=100, orient=tk.HORIZONTAL, bg=THEME['bg_panel'], fg=THEME['text_bright'], highlightthickness=0)
        slider_edges.set(40)
        slider_edges.pack(fill=tk.X)

        def generate_action():
            n = slider_nodes.get()
            m = slider_edges.get()
            self.traffic_graph.generate_random_city(num_nodes=n, num_edges=m)
            self.last_route_result = None
            self.selected_edge = None
            self.f_results_card.pack_forget()
            self._refresh_comboboxes()
            self.visualizer.draw_network(self.traffic_graph)
            dialog.destroy()
            actual_edges = len(self.traffic_graph.graph.edges())
            messagebox.showinfo("Random City", f"Generated connected city network with {n} intersections and {actual_edges} roads!")

        btn_create = tk.Button(
            dialog, text="GENERATE NETWORK", font=('Segoe UI', 10, 'bold'),
            bg=THEME['accent_cyan'], fg='#000000', command=generate_action
        )
        btn_create.pack(pady=12)
